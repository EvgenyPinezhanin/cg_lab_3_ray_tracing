...
vec3 Phong(SIntersection intersect, SLight currLight, float shadow)
{
	...
	float diffuse = abs(dot(light, intersect.Normal));
...
{
	//** LIGHT **//
	light[0].Position = vec3(1.0, -4.0, 1.0);
	light[1].Position = vec3(-1.0, -4.0, 1.0);
	light[2].Position = vec3(0.0, -4.0, 0.0);
	light[3].Position = vec3(1.0, -4.0, -1.0);
	light[4].Position = vec3(-1.0, -4.0, -1.0);

	light[5].Position = vec3(1.0, -3.0, 1.0);
	light[6].Position = vec3(-1.0, -3.0, 1.0);
	light[7].Position = vec3(0.0, -3.0, 0.0);
	light[8].Position = vec3(1.0, -3.0, -1.0);
	light[9].Position = vec3(-1.0, -3.0, -1.0);

	light[10].Position = vec3(1.0, -2.5, 1.0);
	light[11].Position = vec3(-1.0, -2.5, 1.0);

	/** MATERIALS **/
	vec4 lightCoefs = vec4(0.05,0.09,0.1,512.0);
	materials[0].Color = vec3(0.12, 0.87, 0.73);   
	materials[0].LightCoeffs = vec4(lightCoefs);
	materials[0].ReflectionCoef = 0.5;
	materials[0].RefractionCoef = 1.0;
	materials[0].MaterialType = DIFFUSE_REFLECTION;
 
    materials[1].Color = vec3(0.12, 0.32, 0.55);
	materials[1].LightCoeffs = vec4(lightCoefs);
    materials[1].ReflectionCoef = 0.5;
	materials[1].RefractionCoef = 1.0;
	materials[1].MaterialType = DIFFUSE_REFLECTION;
	
	materials[2].Color = vec3(0.34, 0.34, 0.71);  
	...
	materials[3].Color = vec3(0.45, 0.71, 0.97);  
	...
	materials[4].Color = vec3(0.45, 0.66, 0.81);  
	...
	materials[5].Color = vec3(0.18, 1.0, 1.0);  
	...
	// materials[7].RefractionCoef = 0.55;
		...
		if (Raytrace(ray, start, final, intersect))
		{
			switch(intersect.MaterialType)
			{
				case DIFFUSE_REFLECTION:
				{
					for(int i = 0; i < 12; i++)
					{
						float shadowing = Shadow(light[i], intersect);						
						resultColor += trRay.contribution * Phong(intersect, light[i], shadowing);
					}
					break;
				}
				case MIRROR_REFLECTION:
				{
					if(intersect.ReflectionCoef < 1)
					{
						for(int i = 0; i < 12; i++)
						{
							...
							float shadowing = Shadow(light[i], intersect);						 
							resultColor += contribution * Phong(intersect, light[i], shadowing);
						}
					}
                ...
                //     else
                //     {
                //         eta = intersect.RefractionCoef / 1.0;
                //         sign = -1;
                //     }
                //     vec3 refractionDirection = normalize(refract(normalize(ray.Direction), normalize(intersect.Normal * sign), eta));
